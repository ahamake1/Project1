...
public class FOODDriver {
	...
	/**
	 * Method description: Main is the entry point of the application
	 * Date: Jan 18, 2018
	 * @param args
	 * @return void
	 */
	...
	public static void main(String[] args)
	{
		...
		//Utilities Util = new Utilities(); <-- unneeded object utilization
		...
		while(!done)
		{
			...
			if(parseVariable(selection, "int"))
			{
			...
			while(true)
			{
				...
				if(name.equals("F") || name.equals("f"))
				{
					...
					if(parseVariable(itemPrice, "float")==true)
					{
	...
	/**
	 * Method description: Method to handle all needed variable parsing for application
	 * Date: Jan 18, 2018
	 * @param args
	 * @return void
	 */
public static boolean parseVariable(String variable, String type) {
		
		//declare local variables
		boolean isValid = false;
		// if desired variable needs to be parsed as an int...
		if (type.equals("int")) {
			// parse as int
			try {
				//attempt to parse the variable as an integer
				Integer.parseInt(variable);
				//indicate that the variable successfully parsed
				isValid = true;
			}
			//if the variable fails to be parsed as an integer...
			catch(Exception E){
				//indicate that the variable failed to be parsed
				isValid = false;
			}
		}
		// if desired variable needs to be parsed as a float...
		else if(type.equals("float")) {
			// parse as float
			try {
				//attempt to parse the variable as a float
